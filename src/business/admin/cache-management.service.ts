 ...
 * 캐시 관리 서비스
 *
 * CacheEvictionWorker에서 추출한 캐시 상태 조회 및 Eviction 로직.
 * AdminService(API 프로세스)와 CacheEvictionWorker(워커 프로세스) 모두에서 사용 가능.
 *
 * 프로세스 분리 시:
 * - API 프로세스: AdminService -> CacheManagementService (상태 조회 / 수동 eviction)
 * - 워커 프로세스: CacheEvictionWorker -> CacheManagementService (@Cron eviction)
 */
