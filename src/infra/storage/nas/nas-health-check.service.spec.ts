/**
 * ============================================================
 * 📦 NAS Health Check 도메인 서비스 테스트
 * ============================================================
 *
 * 🎯 테스트 대상:
 *   - NasHealthCheckDomainService
 *
 * 📋 비즈니스 맥락:
 *   - Windows UNC 경로를 통한 NAS 연결 상태 확인
 *   - PowerShell을 사용하여 매핑된 드라이브의 용량 정보 조회
 *   - 총 용량, 사용 용량, 여유 용량 정보 제공
 *
 * ⚠️ 중요 고려사항:
 *   - Windows 환경에서만 정상 동작
 *   - UNC 경로가 네트워크 드라이브로 매핑되어 있어야 함
 *   - PowerShell 실행 실패 시 unhealthy 상태 반환
 * ============================================================
 */
import { Test, TestingModule } from '@nestjs/testing';
import { ConfigService } from '@nestjs/config';
import { NasHealthCheckService, NasHealthResult, NasCapacity } from './nas-health-check.service';

describe('NasHealthCheckService', () => {
  let service: NasHealthCheckService;
  let configService: jest.Mocked<ConfigService>;

  /**
   * 🎭 Mock 설정
   * 📍 configService.get('NAS_MOUNT_PATH'):
   *   - 실제 동작: 환경변수에서 NAS UNC 경로 조회
   *   - Mock 이유: 테스트 환경에서 실제 NAS 연결 없이 경로 설정
   */
  beforeEach(async () => {
    configService = {
      get: jest.fn(),
    } as any;

    const module: TestingModule = await Test.createTestingModule({
      providers: [
        NasHealthCheckService,
        {
          provide: ConfigService,
          useValue: configService,
        },
      ],
    }).compile();

    service = module.get<NasHealthCheckService>(
      NasHealthCheckService,
    );
  });

  describe('checkHealth', () => {
    /**
     * 📌 테스트 시나리오: 정상 흐름 - NAS 연결 성공 및 용량 정보 반환
     *
     * 🎯 검증 목적:
     *   NAS가 정상 연결되면 healthy 상태와 함께 용량 정보를 반환해야 한다.
     *   PowerShell을 통해 매핑된 드라이브의 용량을 조회한다.
     *
     * ✅ 기대 결과:
     *   - status = 'healthy'
     *   - capacity에 total, used, free 정보 포함
     *   - checkedAt이 유효한 Date
     */
    it('should return healthy status with capacity when NAS is connected', async () => {
      // ═══════════════════════════════════════════════════════
      // 📥 GIVEN (사전 조건 설정)
      // ═══════════════════════════════════════════════════════
      configService.get.mockReturnValue('\\\\192.168.10.249\\Web');

      // ═══════════════════════════════════════════════════════
      // 🎬 WHEN (테스트 실행)
      // ═══════════════════════════════════════════════════════
      const result = await service.checkHealth();

      // ═══════════════════════════════════════════════════════
      // ✅ THEN (결과 검증)
      // ═══════════════════════════════════════════════════════
      // 실제 NAS 연결에 따라 결과가 달라짐
      // healthy 또는 unhealthy 둘 중 하나
      expect(['healthy', 'degraded', 'unhealthy']).toContain(result.status);
      expect(result.checkedAt).toBeInstanceOf(Date);
      expect(result.responseTimeMs).toBeGreaterThanOrEqual(0);
    });

    /**
     * 📌 테스트 시나리오: 에러 케이스 - NAS 경로 미설정
     *
     * 🎯 검증 목적:
     *   NAS_MOUNT_PATH가 설정되지 않으면 unhealthy 상태를 반환해야 한다.
     *   에러 메시지로 원인을 알 수 있어야 한다.
     *
     * ✅ 기대 결과:
     *   - status = 'unhealthy'
     *   - error에 원인 메시지 포함
     */
    it('should return unhealthy status when NAS_MOUNT_PATH is not configured', async () => {
      // ═══════════════════════════════════════════════════════
      // 📥 GIVEN (사전 조건 설정)
      // ═══════════════════════════════════════════════════════
      configService.get.mockReturnValue(undefined);

      // ═══════════════════════════════════════════════════════
      // 🎬 WHEN (테스트 실행)
      // ═══════════════════════════════════════════════════════
      const result = await service.checkHealth();

      // ═══════════════════════════════════════════════════════
      // ✅ THEN (결과 검증)
      // ═══════════════════════════════════════════════════════
      expect(result.status).toBe('unhealthy');
      expect(result.error).toContain('NAS_MOUNT_PATH');
    });

    /**
     * 📌 테스트 시나리오: 에러 케이스 - 잘못된 UNC 경로
     *
     * 🎯 검증 목적:
     *   유효하지 않은 UNC 경로가 설정되면 unhealthy 상태를 반환해야 한다.
     *
     * ✅ 기대 결과:
     *   - status = 'unhealthy'
     *   - error에 원인 메시지 포함
     */
    it('should return unhealthy status when UNC path is invalid', async () => {
      // ═══════════════════════════════════════════════════════
      // 📥 GIVEN (사전 조건 설정)
      // ═══════════════════════════════════════════════════════
      configService.get.mockReturnValue('invalid-path');

      // ═══════════════════════════════════════════════════════
      // 🎬 WHEN (테스트 실행)
      // ═══════════════════════════════════════════════════════
      const result = await service.checkHealth();

      // ═══════════════════════════════════════════════════════
      // ✅ THEN (결과 검증)
      // ═══════════════════════════════════════════════════════
      expect(result.status).toBe('unhealthy');
    });
  });

  describe('NasHealthResult interface', () => {
    /**
     * 📌 테스트 시나리오: 인터페이스 검증 - 결과 구조 확인
     *
     * 🎯 검증 목적:
     *   NasHealthResult가 올바른 구조를 가지는지 확인한다.
     *   API 응답 형식이 일관되어야 한다.
     *
     * ✅ 기대 결과:
     *   - status, responseTimeMs, checkedAt 필수
     *   - capacity는 healthy 시 선택적으로 포함
     */
    it('should have correct structure in result', async () => {
      // ═══════════════════════════════════════════════════════
      // 📥 GIVEN (사전 조건 설정)
      // ═══════════════════════════════════════════════════════
      configService.get.mockReturnValue('\\\\192.168.10.249\\Web');

      // ═══════════════════════════════════════════════════════
      // 🎬 WHEN (테스트 실행)
      // ═══════════════════════════════════════════════════════
      const result = await service.checkHealth();

      // ═══════════════════════════════════════════════════════
      // ✅ THEN (결과 검증)
      // ═══════════════════════════════════════════════════════
      expect(result).toHaveProperty('status');
      expect(result).toHaveProperty('responseTimeMs');
      expect(result).toHaveProperty('checkedAt');
      // capacity와 error는 상황에 따라 있을 수 있음
    });
  });

  describe('UNC 경로 파싱', () => {
    /**
     * 📌 테스트 시나리오: 다양한 UNC 경로 형식 처리
     *
     * 🎯 검증 목적:
     *   다양한 이스케이프 형식의 UNC 경로가 올바르게 파싱되는지 확인한다.
     *   .env 파일에서 읽어온 경로가 다양한 형태일 수 있기 때문이다.
     *
     * ✅ 기대 결과:
     *   - 정상적인 UNC 경로는 checkHealth가 동작
     *   - 잘못된 형식은 unhealthy 반환
     */
    it.each([
      // [입력 경로, 설명]
      ['\\\\192.168.10.249\\Web', '표준 UNC 경로 (백슬래시 2개)'],
      ['\\\\\\\\192.168.10.249\\\\Web', '이스케이프된 UNC 경로 (백슬래시 4개)'],
      ['//192.168.10.249/Web', '슬래시 형식 UNC 경로'],
      ['\\\\192.168.10.249\\Web\\personal\\서우혁\\dms', '하위 폴더 포함 UNC 경로'],
      ['\\\\\\\\192.168.10.249\\\\Web\\\\personal\\\\서우혁\\\\dms', '이스케이프된 하위 폴더 포함'],
    ])('should handle UNC path: %s (%s)', async (uncPath, _description) => {
      // ═══════════════════════════════════════════════════════
      // 📥 GIVEN (사전 조건 설정)
      // ═══════════════════════════════════════════════════════
      configService.get.mockReturnValue(uncPath);

      // ═══════════════════════════════════════════════════════
      // 🎬 WHEN (테스트 실행)
      // ═══════════════════════════════════════════════════════
      const result = await service.checkHealth();

      // ═══════════════════════════════════════════════════════
      // ✅ THEN (결과 검증)
      // ═══════════════════════════════════════════════════════
      // 파싱은 성공해야 함 (실제 NAS 연결 실패는 별개)
      // 파싱 실패 시 에러 메시지에 '파싱 실패' 포함
      if (result.error) {
        expect(result.error).not.toContain('파싱 실패');
      }
      expect(result).toHaveProperty('status');
    });

    /**
     * 📌 테스트 시나리오: 잘못된 UNC 경로 - 서버만 있는 경우
     *
     * 🎯 검증 목적:
     *   공유 폴더 없이 서버만 있는 경로는 파싱 실패해야 한다.
     *
     * ✅ 기대 결과:
     *   - status = 'unhealthy'
     *   - error에 파싱 실패 메시지 포함
     */
    it('should fail when UNC path has only server without share', async () => {
      // ═══════════════════════════════════════════════════════
      // 📥 GIVEN (사전 조건 설정)
      // ═══════════════════════════════════════════════════════
      configService.get.mockReturnValue('\\\\192.168.10.249');

      // ═══════════════════════════════════════════════════════
      // 🎬 WHEN (테스트 실행)
      // ═══════════════════════════════════════════════════════
      const result = await service.checkHealth();

      // ═══════════════════════════════════════════════════════
      // ✅ THEN (결과 검증)
      // ═══════════════════════════════════════════════════════
      expect(result.status).toBe('unhealthy');
      expect(result.error).toContain('파싱 실패');
    });
  });
});
