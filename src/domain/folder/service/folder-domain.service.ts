...
export class FolderDomainService {
   ...
   * ID로 폴더 조회
   */
  async 조회(folderId: string): Promise<FolderEntity | null> {
   ...
   * @returns 생성된 폴더 엔티티
   */
  async 생성(params: CreateFolderParams): Promise<FolderEntity> {
    // 경로가 전달되지 않은 경우 직접 계산
    let folderPath = params.path;
    if (!folderPath) {
      let parentPath = '';
      if (params.parentId) {
        const parent = await this.folderRepository.findById(params.parentId);
        if (!parent) {
   ...
   * @throws Error - 폴더가 존재하지 않거나 활성 상태가 아닌 경우
   */
  async 이름변경(folderId: string, newName: string, newPath?: string): Promise<FolderEntity> {
    const folder = await this.folderRepository.findByIdForUpdate(folderId);
    if (!folder) {
      throw new Error(`폴더를 찾을 수 없습니다: ${folderId}`);
    }
   ...
   * @throws Error - 폴더가 존재하지 않거나 활성 상태가 아닌 경우
   */
  async 이동(folderId: string, targetParentId: string, newPath?: string): Promise<FolderEntity> {
    const folder = await this.folderRepository.findByIdForUpdate(folderId);
    if (!folder) {
      throw new Error(`폴더를 찾을 수 없습니다: ${folderId}`);
    }
   ...
   */
  async 영구삭제(folderId: string): Promise<FolderEntity> {
    const folder = await this.folderRepository.findByIdForUpdate(folderId);
    if (!folder) {
      throw new Error(`폴더를 찾을 수 없습니다: ${folderId}`);
    }
   ...
   * @returns 영향받은 폴더 수
   */
  async 경로일괄변경(oldPrefix: string, newPrefix: string): Promise<number> {
    return this.folderRepository.updatePathByPrefix(oldPrefix, newPrefix);
  }
}
